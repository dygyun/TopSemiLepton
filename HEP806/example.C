...
int main() {
   bool run = true;
 if (run) {
   //float lumi = 16146.177653168; 
   float lumi    = 35867.059982506; 
   TString dir = "/xrootd/store/user/dygyun/Cattools/Catv806_scEta/";
   ...
   int nevt = -1;
   MyAnalysis *A = new MyAnalysis();  //// only for data
   ...
   MyAnalysis_others *C = new MyAnalysis_others(1,1,61526.7,lumi,hC->GetBinContent(2));
   TChain* ch3 = new TChain("TopTree/events");
   ch3->Add(Form("%s/hep_WJets.root",dir.Data()),nevt);
   ch3->Process(C);
   
   MyAnalysis_others *D = new MyAnalysis_others(1,1,6025.2,lumi,hD->GetBinContent(1));
   TChain* ch4 = new TChain("TopTree/events");
   ch4->Add(Form("%s/hep_DYJets.root",dir.Data()),nevt);
   ch4->Process(D);

   MyAnalysis_others *E = new MyAnalysis_others(1,1,18610.0,lumi,hE->GetBinContent(1));
   TChain* ch5 = new TChain("TopTree/events");
   ch5->Add(Form("%s/hep_DYJets_10to50.root",dir.Data()),nevt);
   ch5->Process(E);
 
   MyAnalysis_others *I = new MyAnalysis_others(1,1,44.33,lumi,hI->GetBinContent(1));
   TChain* ch6 = new TChain("TopTree/events");
   ch6->Add(Form("%s/hep_SingleTop_t.root",dir.Data()),nevt);
   ch6->Process(I);

   MyAnalysis_others *J = new MyAnalysis_others(1,1,26.38,lumi,hJ->GetBinContent(1));
   TChain* ch7 = new TChain("TopTree/events");
   ch7->Add(Form("%s/hep_SingleTbar_t.root",dir.Data()),nevt);
   ch7->Process(J);

   MyAnalysis_others *K = new MyAnalysis_others(1,1,35.85,lumi,hK->GetBinContent(1));
   TChain* ch8 = new TChain("TopTree/events");
   ch8->Add(Form("%s/hep_SingleTop_tW.root",dir.Data()),nevt);
   ch8->Process(K);

   MyAnalysis_others *L = new MyAnalysis_others(1,1,35.85,lumi,hL->GetBinContent(1));
   TChain* ch9 = new TChain("TopTree/events");
   ch9->Add(Form("%s/hep_SingleTbar_tW.root",dir.Data()),nevt);
   ch9->Process(L);

   MyAnalysis_others *N = new MyAnalysis_others(1,1,118.7,lumi,hN->GetBinContent(1));
   TChain* ch10 = new TChain("TopTree/events");
   ch10->Add(Form("%s/hep_WW.root",dir.Data()),nevt);
   ch10->Process(N);

   MyAnalysis_others *O = new MyAnalysis_others(1,1,47.13,lumi,hO->GetBinContent(1));
   TChain* ch11 = new TChain("TopTree/events");
   ch11->Add(Form("%s/hep_WZ.root",dir.Data()),nevt);
   ch11->Process(O);
   
   MyAnalysis_others *M = new MyAnalysis_others(1,1,16.523,lumi,hM->GetBinContent(1));
 ...
 // VV 
  ...
  N->histograms[i]->Add(M->histograms[i]); 
   ...
   P.Plot("results_Mujet_Total_Acceptance.pdf");

   TFile * outA = TFile::Open("hist_SingleMuon_data.root","RECREATE");
