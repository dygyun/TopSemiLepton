...
//  edm::EDGetTokenT<float>                        CSVWeightToken_;
 // edm::EDGetTokenT<std::vector<float>>           CSVSysWeightToken_;
  ...
  float Jet_JER_Nom[kMax];
  float Jet_JER_Up[kMax];
  float Jet_JER_Dw[kMax];
...
TopAnalyzer::TopAnalyzer(const edm::ParameterSet& iConfig)
{
  ...
  puUpWeightToken_ = consumes<float>                        (iConfig.getParameter<edm::InputTag>("puWeightUp"));
  puDownWeightToken_= consumes<float>                       (iConfig.getParameter<edm::InputTag>("puWeightDown"));
...
void TopAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(!iEvent.isRealData()) {
    ...
    iEvent.getByToken(puUpWeightToken_, PileUpWeight_Up);
    PUWeightUp = *PileUpWeight_Up;

    edm::Handle<float> PileUpWeight_Down;
    iEvent.getByToken(puDownWeightToken_, PileUpWeight_Down);
    PUWeightDown = *PileUpWeight_Down;
...
////////
  ...
  for (unsigned int i = 0; i < electrons->size() ; i++) {
    ...
    bool passTightElectron = electron.pt() > 30 && fabs(electron.eta()) < 2.1 && electron.electronID("cutBasedElectronID-Summer16-80X-V1-medium") > 0;
...
//////////////////////////////////////////
  ...
  for (unsigned int i = 0; i < jets->size() ; i++) {
    ...
    TLorentzVector vjet;
      //vjet.SetPxPyPzE(jet.pt(), jet.eta(), jet.phi(), jet.energy() );
    vjet.SetPtEtaPhiE(jet.pt(), jet.eta(), jet.phi(), jet.energy() );

  //  vjet = vjet * jet.smearedResDown(); 
    //vjet = vjet * jet.smearedRes(); 
 //   cout << "  jet.pt()  = " << jet.pt() << "=======__jet.eta()+++ " << jet.eta()<< endl;
    bool pass = std::abs(vjet.Eta()) < 2.4 && vjet.Pt()  > 30 && jet.LooseId() ;
    if (!pass ) continue; 
  
    double dr = 999.9;
   // std::cout << "  jet.pt()  = " << vjet.Pt()  << "========_jet.eta()+++ "<<  vjet.Eta() << "______Jet_Pt[nJets]--------" << std::endl;
...
////////////////////
    ...
    Jet_Pt[nJets]  = vjet.Pt();
    Jet_Eta[nJets] = vjet.Eta();
    Jet_Phi[nJets] = vjet.Phi();
    Jet_E[nJets]   = vjet.E();
    ...
    Jet_JER_Nom[nJets] = jet.smearedRes();
    Jet_JER_Up[nJets]  = jet.smearedResUp();
    Jet_JER_Dw[nJets]  = jet.smearedResDown();
////// for JER UP 
    ...
    if( bDiscriminator > WP_BTAG_CSVv2T) 
    {
      nbJets++;
      Jet_BTag[nJets] = 1;
    }else{
...
void TopAnalyzer::beginJob()
{
  ...
  tree->Branch("Jet_JER_Nom",Jet_JER_Nom,"Jet_JER_Nom[NJet]/F");
  tree->Branch("Jet_JER_Up",Jet_JER_Up,"Jet_JER_Up[NJet]/F");
  tree->Branch("Jet_JER_Dw",Jet_JER_Dw,"Jet_JER_Dw[NJet]/F");
