...
void MyAnalysis_Sig::BuildEvent() {
...
void MyAnalysis_Sig::Begin(TTree * /*tree*/) {
...
void MyAnalysis_Sig::SlaveBegin(TTree * /*tree*/) {
   // The SlaveBegin() function is called after the Begin() function.
   TString option = GetOption();

   for(int i=0; i < 5; i++){ 
     hIso_vs_MET[i] = new TH2D(Form("hIso_vs_MET%i",i), "Lepton PF Isolation vs #slash{E}_{T}", 50 , 0, 200 , 100, 0 , 2.0 );
     ...
     h_NBJet[i] = new TH1D(Form("h_NBJet_S%i",i), "Number of b tagged jets", 5, 0, 5);
   ...
   Bool_t MyAnalysis_Sig::Process(Long64_t entry) {
  ...
  if ( SemiLeptonic == 1 ) { 
   ...
   int sel_mu = 0;
   int imu = 0;
   int noniso_mu = 0; // for no isolated muon 
   int NumMu_abcd = 0; // number of no iso muon
...
///////
     if ( jt->Pt() > MuonPtCut && fabs( jt->Eta() ) < 2.4 )
     {
        N_noIsoMuon++;
       if( N_noIsoMuon == 1 ) 
        {
          muon1 = &(*jt);
          noniso_mu = NumMu_abcd;
        }
     }
     imu++;
     NumMu_abcd++;
...
//////////
   ...
   //step 0 
   if( N_noIsoMuon == 1 ){
     hIso_vs_MET[0]->Fill(MET, Muon_Iso03[0], EventWeight);
     h_Muon_Pt[0]->Fill(Muon_Pt[noniso_mu], EventWeight);
     h_MuonIso[0]->Fill(Muon_Iso03[0], EventWeight);
     h_NMuon[0]->Fill(N_noIsoMuon, EventWeight);
     h_nAccMuon[0]->Fill(N_IsoMuon, Acc_EventWeight);
     h_NVertex[0]->Fill(NVertex, EventWeight);
     h_WMuon_MT[0]->Fill( WMuon_MT[noniso_mu], EventWeight);
...
/////////////Combine BF to GH
    ...
    //  cout << "muBF" << lumi_BF/lumi << "_______mu GH: " << lumi_GH/lumi << "MU SF"<< musf_tot  << "___muIsosf_tot" << muIsosf_tot << "++++muTrigsf_tot"<<  muTrigsf_tot << endl;
     ...
     hIso_vs_MET[1]->Fill(MET, Muon_Iso03[noniso_mu], EventWeight);
     h_Muon_Pt[1]->Fill(Muon_Pt[noniso_mu], EventWeight);
     h_MuonIso[1]->Fill(Muon_Iso03[noniso_mu], EventWeight); 
     h_NMuon[1]->Fill(N_noIsoMuon, EventWeight);
     h_nAccMuon[1]->Fill(N_IsoMuon, Acc_EventWeight);
     h_NVertex[1]->Fill(NVertex, EventWeight);
     h_WMuon_MT[1]->Fill( WMuon_MT[noniso_mu], EventWeight);
   ...
   //  if (N_IsoMuon > 1 ) h_Mmumu[1]->Fill((*muon1 + *muon2).M(), EventWeight);
     ...
     if( NJet > 3){ //step 2
       hIso_vs_MET[2]->Fill(MET, Muon_Iso03[noniso_mu], EventWeight);
       h_Muon_Pt[2]->Fill(Muon_Pt[noniso_mu], EventWeight);
       h_MuonIso[2]->Fill(Muon_Iso03[noniso_mu], EventWeight); 
       h_NMuon[2]->Fill(N_noIsoMuon, EventWeight);
       h_nAccMuon[2]->Fill(N_IsoMuon, Acc_EventWeight);
       h_NVertex[2]->Fill(NVertex, EventWeight);
       h_WMuon_MT[2]->Fill( WMuon_MT[noniso_mu], EventWeight);
       ...
       if( NBJet > 1 ){ //step 3
         hIso_vs_MET[3]->Fill(MET, Muon_Iso03[noniso_mu], EventWeight);
         h_Muon_Pt[3]->Fill(Muon_Pt[noniso_mu], EventWeight);
         h_MuonIso[3]->Fill(Muon_Iso03[noniso_mu], EventWeight); 
         h_NMuon[3]->Fill(N_noIsoMuon, EventWeight);
         h_nAccMuon[3]->Fill(N_IsoMuon, Acc_EventWeight);
         h_NVertex[3]->Fill(NVertex, EventWeight);
         h_WMuon_MT[3]->Fill( WMuon_MT[noniso_mu], EventWeight);
         ...
         if(  Muon_Iso03[noniso_mu] < 0.1  ){ 
           hIso_vs_MET[4]->Fill(MET, Muon_Iso03[noniso_mu], EventWeight);
           h_Muon_Pt[4]->Fill(Muon_Pt[noniso_mu], EventWeight);
           h_MuonIso[4]->Fill(Muon_Iso03[noniso_mu], EventWeight);
           h_NMuon[4]->Fill(N_noIsoMuon, EventWeight);
           h_nAccMuon[4]->Fill(N_IsoMuon, Acc_EventWeight);
           h_NVertex[4]->Fill(NVertex, EventWeight);
           h_WMuon_MT[4]->Fill( WMuon_MT[noniso_mu], EventWeight);
...
void MyAnalysis_Sig::SlaveTerminate() {
}
void MyAnalysis_Sig::Terminate() {
